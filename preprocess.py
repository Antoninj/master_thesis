...
class DataPreprocessor(object):
    """ Class that handles all the preprocessing of the acquisition signal using the open source library Scipy.
    ...
    def apply_filtering(self, input_signal, analog_frequency):
        ...
        # Apply the filter
        filtered_signal = scipy.signal.filtfilt(b, a, input_signal[:, 0])
    ...
    def apply_detrending(self, input_signal):
        ...
        detrending_type = self.config["type"]

        return scipy.signal.detrend(input_signal, detrending_type)
    ...
    def preprocess(self, input_signal, analog_frequency, balance_board=False):
        ...
        filtered_signal = self.apply_filtering(input_signal, analog_frequency)
